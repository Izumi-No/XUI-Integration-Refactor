...
export class UpdateUserController implements Controller{
    ...
    async handle({paramsUsername, userOutputs, ...data}: UpdateUserControllerRequest): Promise<HttpResponse>{
        try{
            return ok(await this.userService.updateByUsername(paramsUsername, data, userOutputs))
        }catch(e){
            fail(e)
        }
    }
